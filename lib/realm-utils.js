...
export function validateSchema(schema, normalizedSchema) {
  for (const objectSchema of schema) {
    const normalizedObjectSchema = normalizedSchema.find(
      (normalizedObjectSchema) =>
        normalizedObjectSchema.name === objectSchema.name
    )
    if (!normalizedObjectSchema) {
      throw new Error(
        `object '${objectSchema.name}' is not in the schema in the database`
      )
    }
    if (!deepEqual(normalizeSchema(objectSchema), normalizedObjectSchema)) {
      throw new Error(
        `the schema for '${objectSchema.name}' does not match the schema in the database`
      )
    }
  }
}
